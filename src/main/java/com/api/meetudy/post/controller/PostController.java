...
public class PostController {
    ...
    public ResponseEntity<ApiResponse<List<PostDto>>> getAllPosts() {
        ...
        return ResponseEntity.ok(ApiResponse.onSuccess(postDtoList));
    }

    @Operation(summary = "특정 사용자의 게시글 조회 API")
    @GetMapping("/member")
    public ResponseEntity<ApiResponse<List<PostDto>>> getPostsByMember(Principal principal) {
        List<PostDto> postDtoList = postService.getPostsByMember(authenticationService.getCurrentMember(principal));
        return ResponseEntity.ok(ApiResponse.onSuccess(postDtoList));
    }
